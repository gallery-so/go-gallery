...
func PipelineAssetsForAcc(pCtx context.Context, pUserID persist.DBID, pOwnerWalletAddresses []persist.Address,
	...
	if len(result) == 0 {
		return nil, fmt.Errorf("no NFTs found for user %s", pUserID)
	}

	return result, nil
}

func openseaSyncHistories(pCtx context.Context, pNfts []*persist.NFT, userRepo persist.UserRepository, nftRepo persist.NFTRepository, historyRepo persist.OwnershipHistoryRepository) ([]*persist.NFT, error) {
...
func openseaFetchAssetsForWallets(pCtx context.Context, pWalletAddresses []persist.Address, nftRepo persist.NFTRepository) ([]persist.NFT, error) {
	result := []persist.NFT{}
...
func fillNFTIDs(pCtx context.Context, pNfts []persist.NFT, pUser persist.User, nftRepo persist.NFTRepository) ([]persist.NFT, error) {
