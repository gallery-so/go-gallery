...
func tokensToTokensGallery(tokens []persist.Token) []persist.TokenGallery {
	...
	for i, token := range tokens {
		res[i] = persist.TokenGallery{
			...
			ContractAddress:  persist.Address{Address: persist.NullString(token.ContractAddress.String()), Chain: persist.ChainETH},
			OwnerAddress:     persist.Address{Address: persist.NullString(token.OwnerAddress.String()), Chain: persist.ChainETH},
...
func contractToContractGallery(contract persist.Contract) persist.ContractGallery {
	return persist.ContractGallery{
		Address:        persist.Address{Address: persist.NullString(contract.Address.String()), Chain: persist.ChainETH},
		Name:           contract.Name,
...
func ethereumAddressAtBlockToGallery(addrs []persist.EthereumAddressAtBlock) []persist.AddressAtBlock {
	...
	for i, addr := range addrs {
		res[i] = persist.AddressAtBlock{
			Address: persist.Address{Address: persist.NullString(addr.Address.String()), Chain: persist.ChainETH},
			Block:   addr.Block,
		}
	}
	return res
