...
func (f *FeatureFlagRepository) GetByRequiredTokens(pCtx context.Context, pRequiredTokens map[persist.TokenIdentifiers]uint64) ([]persist.FeatureFlag, error) {
	...
	getFlagsSQLStr := `SELECT ID,VERSION,DELETED,LAST_UPDATED,CREATED_AT,REQUIRED_TOKEN,REQUIRED_AMOUNT,TOKEN_TYPE,NAME,IS_ENABLED,ADMIN_ONLY,FORCE_ENABLED_USER_IDS FROM feature WHERE REQUIRED_TOKEN = ANY($1)`
...
func (f *FeatureFlagRepository) GetByName(pCtx context.Context, pName string) (persist.FeatureFlag, error) {
	getFlagSQLStr := `SELECT ID,VERSION,DELETED,LAST_UPDATED,CREATED_AT,REQUIRED_TOKEN,REQUIRED_AMOUNT,TOKEN_TYPE,NAME,IS_ENABLED,ADMIN_ONLY,FORCE_ENABLED_USER_IDS FROM feature WHERE NAME = $1`
	var flag persist.FeatureFlag
...
func (f *FeatureFlagRepository) GetAll(pCtx context.Context) ([]persist.FeatureFlag, error) {
	getFlagsSQLStr := `SELECT ID,VERSION,DELETED,LAST_UPDATED,CREATED_AT,REQUIRED_TOKEN,REQUIRED_AMOUNT,TOKEN_TYPE,NAME,IS_ENABLED,ADMIN_ONLY,FORCE_ENABLED_USER_IDS FROM feature`
