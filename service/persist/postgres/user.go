...
func NewUserRepository(db *sql.DB) *UserRepository {
	...
	getByWalletIDStmt, err := db.PrepareContext(ctx, `SELECT ID,DELETED,VERSION,USERNAME,USERNAME_IDEMPOTENT,WALLETS,BIO,TRAITS,UNIVERSAL,CREATED_AT,LAST_UPDATED FROM users WHERE ARRAY[$1] <@ WALLETS AND DELETED = false;`)
	checkNoErr(err)
