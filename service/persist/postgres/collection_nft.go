...
func NewCollectionRepository(db *sql.DB, galleryRepo *GalleryRepository) *CollectionRepository {
	...
	FROM collections c,unnest(c.NFTS) WITH ORDINALITY AS u(nft, ordinality) 
	LEFT JOIN nfts n ON n.ID = nft
	WHERE c.OWNER_USER_ID = $1 AND c.HIDDEN = false AND c.DELETED = false AND n.DELETED = false ORDER BY ordinality;`)
	checkNoErr(err)
	...
	FROM collections c, unnest(c.NFTS) WITH ORDINALITY AS u(nft, ordinality)
	LEFT JOIN nfts n ON n.ID = nft
	WHERE c.ID = $1 AND c.HIDDEN = false AND c.DELETED = false AND n.DELETED = false ORDER BY ordinality;`)
	checkNoErr(err)
...
func (c *CollectionRepository) RemoveNFTsOfOldAddresses(pCtx context.Context, pUserID persist.DBID) error {
	...
	if err := c.getUserWalletsStmt.QueryRowContext(pCtx, pUserID).Scan(pq.Array(&addresses)); err != nil {
		return err
	}
