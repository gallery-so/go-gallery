...
func (c *ContractRepository) GetByAddress(pCtx context.Context, pAddress persist.Address) (persist.Contract, error) {
	sqlStr := `SELECT ID,VERSION,CREATED_AT,LAST_UPDATED,ADDRESS,SYMBOL,NAME,LATEST_BLOCK FROM contracts WHERE ADDRESS = $1 AND DELETED = false`
	contract := persist.Contract{}
	err := c.db.QueryRowContext(pCtx, sqlStr, pAddress).Scan(&contract.ID, &contract.Version, &contract.CreationTime, &contract.LastUpdated, &contract.Address, &contract.Symbol, &contract.Name, &contract.LatestBlock)
	if err != nil {
