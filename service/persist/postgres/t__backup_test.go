...
func TestBackup(t *testing.T) {
	...
	t.Run("backups are pruned correctly", func(t *testing.T) {
		...
		now := time.Now()
		d1 := now.Add(-6 * time.Minute)
		d2 := now.Add(-7 * time.Minute)
		d3 := now.Add(-15 * time.Minute)
		d4 := now.Add(-25 * time.Minute)
		d5 := now.Add(-1*time.Hour + 5*time.Minute)
		d6 := now.Add(-2 * time.Hour)
		d7 := now.Add(-2*time.Hour - 1*time.Minute)
		d8 := now.Add(-15 * time.Hour)
		// timestamps within past 7d
		day := 24 * time.Hour
		w1 := now.Add(-3 * day)
		w2 := now.Add(-3*day - 20*time.Minute)
		w3 := now.Add(-3*day - 30*time.Minute)
		w4 := now.Add(-3*day - 40*time.Minute)
		w5 := now.Add(-3*day - 120*time.Minute)
		w6 := now.Add(-5 * day)
		w7 := now.Add(-6*day - 2*time.Hour)
		// timestamps beyond 7d
		t1 := now.Add(-10 * day)
		t2 := now.Add(-10*day - 30*time.Minute)
		t3 := now.Add(-11*day - 10*time.Minute)
		t4 := now.Add(-12*day - 30*time.Minute)
		t5 := now.Add(-13 * day)
		t6 := now.Add(-30 * day)
		t7 := now.Add(-60 * day)
