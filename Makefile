...
migrate-dev-coredb: start-dev-sql-proxy confirm-dev-migrate
	migrate -path ./db/migrations/core -database "postgresql://$(POSTGRES_MIGRATION_USER):$(POSTGRES_PASSWORD)@localhost:6643/$(POSTGRES_DB)?sslmode=disable" up
...
migrate-prod-coredb: start-prod-sql-proxy confirm-prod-migrate
	migrate -path ./db/migrations/core -database "postgresql://$(POSTGRES_MIGRATION_USER):$(POSTGRES_PASSWORD)@localhost:6543/$(POSTGRES_DB)?sslmode=disable" up
