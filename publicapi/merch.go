...
func (api MerchAPI) GetMerchTokens(ctx context.Context, address persist.Address) ([]*model.MerchToken, error) {
	...
	tokens, err := api.multichainProvider.GetTokensOfContractForWallet(ctx, persist.NewChainAddress(persist.Address(merchAddress), persist.ChainETH), persist.NewL1ChainAddress(address, persist.ChainETH))
	if err != nil {
		return nil, err
	}
