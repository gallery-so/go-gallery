...
func handlersInitServer(router *gin.Engine, mc *multichain.Provider, repos *postgres.Repositories, ethClient *ethclient.Client, ipfsClient *shell.Shell, arweaveClient *goar.Client, stg *storage.Client, tokenBucket string, throttler *throttle.Locker) *gin.Engine {
	...
	mediaGroup.POST("/process", processMediaForUsersTokensOfChain(repos.TokenRepository, repos.ContractRepository, ethClient, ipfsClient, arweaveClient, stg, tokenBucket, throttler))
	mediaGroup.POST("/process/token", processMediaForToken(repos.TokenRepository, repos.UserRepository, repos.WalletRepository, ethClient, ipfsClient, arweaveClient, stg, tokenBucket, throttler))
