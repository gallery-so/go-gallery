...
func processOwnersForUserTokens(mc *multichain.Provider) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		logger.For(c).WithFields(logrus.Fields{"user_id": input.UserID, "total_tokens": len(input.TokenIdentifiers)}).Infof("Processing: %s - Processing User Tokens Refresh (total: %d)", input.UserID, len(input.TokenIdentifiers))
