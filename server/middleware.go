...
func handleCORS(runtimeConfig *runtime.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if util.Contains(allowedOrigins, requestOrigin) || (strings.ToLower(runtime.Config.Env) == "development" && strings.HasPrefix(requestOrigin, "https://gallery-git-") && strings.HasSuffix(requestOrigin, "-gallery-so.vercel.app")) {
