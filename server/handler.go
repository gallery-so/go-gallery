...
func nftHandlersInit(parent *gin.RouterGroup, repos *repositories, ethClient *eth.Client) {
	...
	nftsGroup.GET("/opensea/get", jwtRequired(), getNftsFromOpensea(repos.nftRepository, repos.userRepository, repos.collectionRepository, repos.historyRepository))
	nftsGroup.POST("/opensea/refresh", rateLimited(), jwtRequired(), refreshOpenseaNFTs(repos.nftRepository, repos.userRepository))
