...
func AuthUserLoginAndMemorizeAttemptPipeline(pInput *GLRYauthUserVerifySignatureInput,
	pReq     *http.Request,
	...
	gErr = glry_db.AuthUserLoginAttempt(loginAttempt, pCtx, pRuntime)
	if gErr != nil {
		return false, "", gErr
	}
...
func AuthUserLoginPipeline(pInput *GLRYauthUserVerifySignatureInput,
	pCtx     context.Context,
	pRuntime *glry_core.Runtime) (bool, string, string, *gfcore.Gf_error) {
	...
	return validBool, jwtStr, nonce.ValueStr, nil
}
...
func AuthVerifySignature(pSignatureStr string,
	...
	if err != nil {
		gErr := gfcore.Error__create("failed to hex decode a ECDSA signature",
			"crypto_hex_decode",
	...
	if err != nil {
		gErr := gfcore.Error__create("failed to recover PublicKey from ECDSA public key",
	...
	//------------------
	// VERIFY
	verifiedBool := crypto.VerifySignature(publicKeyBytesLst, dataHash.Bytes(), signatureNoRecoverIDbytesLst)
...
func AuthUserGetPublicInfoPipeline(pInput *GLRYauthUserGetPublicInfoInput,
	pCtx     context.Context,
	pRuntime *glry_core.Runtime) (*GLRYauthUserGetPublicInfoOutput, *gfcore.Gf_error) {
