...
type ContractsLoaderByChainAddressSettings interface {
...
func (l *ContractsLoaderByChainAddress) setContext(ctx context.Context) {
	l.ctx = ctx
}

func (l *ContractsLoaderByChainAddress) setWait(wait time.Duration) {
	l.wait = wait
}

func (l *ContractsLoaderByChainAddress) setMaxBatch(maxBatch int) {
	l.maxBatch = maxBatch
}

func (l *ContractsLoaderByChainAddress) setDisableCaching(disableCaching bool) {
	l.disableCaching = disableCaching
}

func (l *ContractsLoaderByChainAddress) setPublishResults(publishResults bool) {
	l.publishResults = publishResults
}

func (l *ContractsLoaderByChainAddress) setPreFetchHook(preFetchHook func(context.Context, string) context.Context) {
	l.preFetchHook = preFetchHook
}

func (l *ContractsLoaderByChainAddress) setPostFetchHook(postFetchHook func(context.Context, string)) {
	l.postFetchHook = postFetchHook
}

// NewContractsLoaderByChainAddress creates a new ContractsLoaderByChainAddress with the given settings, functions, and options
func NewContractsLoaderByChainAddress(
	settings ContractsLoaderByChainAddressSettings, fetch func(ctx context.Context, keys []coredb.GetContractsByAddressBatchPaginateParams) ([][]coredb.Contract, []error),
	opts ...func(interface {
...
) *ContractsLoaderByChainAddress {
	loader := &ContractsLoaderByChainAddress{
	...
	loader.fetch = func(keys []coredb.GetContractsByAddressBatchPaginateParams) ([][]coredb.Contract, []error) {
		ctx := loader.ctx

		// Allow the preFetchHook to modify and return a new context
		if loader.preFetchHook != nil {
			ctx = loader.preFetchHook(ctx, "ContractsLoaderByChainAddress")
		}

		results, errors := fetch(ctx, keys)

		if loader.postFetchHook != nil {
			loader.postFetchHook(ctx, "ContractsLoaderByChainAddress")
		}
...
// ContractsLoaderByChainAddress batches and caches requests
type ContractsLoaderByChainAddress struct {
	// context passed to fetch functions
	ctx context.Context

	// this method provides the data for the loader
	fetch func(keys []coredb.GetContractsByAddressBatchPaginateParams) ([][]coredb.Contract, []error)
	...
	cache map[coredb.GetContractsByAddressBatchPaginateParams][]coredb.Contract

	// typed cache functions
	//subscribers []func([]coredb.Contract)
	subscribers []contractsLoaderByChainAddressSubscriber
	...
	batch *contractsLoaderByChainAddressBatch
...
type contractsLoaderByChainAddressBatch struct {
	keys    []coredb.GetContractsByAddressBatchPaginateParams
...
func (l *ContractsLoaderByChainAddress) Load(key coredb.GetContractsByAddressBatchPaginateParams) ([]coredb.Contract, error) {
...
func (l *ContractsLoaderByChainAddress) LoadThunk(key coredb.GetContractsByAddressBatchPaginateParams) func() ([]coredb.Contract, error) {
	l.mu.Lock()
	if !l.disableCaching {
	...
	if l.batch == nil {
		l.batch = &contractsLoaderByChainAddressBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *ContractsLoaderByChainAddress) LoadAll(keys []coredb.GetContractsByAddressBatchPaginateParams) ([][]coredb.Contract, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *ContractsLoaderByChainAddress) LoadAllThunk(keys []coredb.GetContractsByAddressBatchPaginateParams) func() ([][]coredb.Contract, []error) {
	results := make([]func() ([]coredb.Contract, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *ContractsLoaderByChainAddress) Prime(key coredb.GetContractsByAddressBatchPaginateParams, value []coredb.Contract) bool {
	if l.disableCaching {
		return false
	}
	l.mu.Lock()
...
func (l *ContractsLoaderByChainAddress) Clear(key coredb.GetContractsByAddressBatchPaginateParams) {
	if l.disableCaching {
		return
	}
	l.mu.Lock()
...
func (l *ContractsLoaderByChainAddress) unsafeSet(key coredb.GetContractsByAddressBatchPaginateParams, value []coredb.Contract) {
	if l.cache == nil {
		l.cache = map[coredb.GetContractsByAddressBatchPaginateParams][]coredb.Contract{}
	}
	l.cache[key] = value
}
...
func (b *contractsLoaderByChainAddressBatch) keyIndex(l *ContractsLoaderByChainAddress, key coredb.GetContractsByAddressBatchPaginateParams) int {
...
func (b *contractsLoaderByChainAddressBatch) startTimer(l *ContractsLoaderByChainAddress) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *contractsLoaderByChainAddressBatch) end(l *ContractsLoaderByChainAddress) {
	b.data, b.error = l.fetch(b.keys)
	close(b.done)
}

type contractsLoaderByChainAddressSubscriber struct {
	cacheFunc func(coredb.Contract)
	mutex     *sync.Mutex
}

func (l *ContractsLoaderByChainAddress) publishToSubscribers(value []coredb.Contract) {
		...
		for i, subscription := range *l.subscriptionRegistry {
			if typedFunc, ok := subscription.(*func(coredb.Contract)); ok {
				...
				if !l.ownsCacheFunc(typedFunc) {
					l.subscribers = append(l.subscribers, contractsLoaderByChainAddressSubscriber{cacheFunc: *typedFunc, mutex: (*l.mutexRegistry)[i]})
				}
			}
		}
	})
...
func (l *ContractsLoaderByChainAddress) registerCacheFunc(cacheFunc interface{}, mutex *sync.Mutex) {
...
func (l *ContractsLoaderByChainAddress) ownsCacheFunc(f *func(coredb.Contract)) bool {
