...
func (l *CommunityLoaderByKey) setContext(ctx context.Context) {
	l.ctx = ctx
}

func (l *CommunityLoaderByKey) setWait(wait time.Duration) {
	l.wait = wait
}

func (l *CommunityLoaderByKey) setMaxBatch(maxBatch int) {
	l.maxBatch = maxBatch
}

func (l *CommunityLoaderByKey) setDisableCaching(disableCaching bool) {
	l.disableCaching = disableCaching
}

func (l *CommunityLoaderByKey) setPublishResults(publishResults bool) {
	l.publishResults = publishResults
}

func (l *CommunityLoaderByKey) setPreFetchHook(preFetchHook func(context.Context, string) context.Context) {
	l.preFetchHook = preFetchHook
}

func (l *CommunityLoaderByKey) setPostFetchHook(postFetchHook func(context.Context, string)) {
...
func NewCommunityLoaderByKey(
	...
	loader.fetch = func(keys []persist.CommunityKey) ([]coredb.Community, []error) {
		ctx := loader.ctx

		// Allow the preFetchHook to modify and return a new context
		if loader.preFetchHook != nil {
			ctx = loader.preFetchHook(ctx, "CommunityLoaderByKey")
		}

		results, errors := fetch(ctx, keys)

		if loader.postFetchHook != nil {
			loader.postFetchHook(ctx, "CommunityLoaderByKey")
		}
	...
	if !loader.disableCaching {
		...
		if funcs.AutoCacheWithKey != nil {
			cacheFunc := func(t coredb.Community) {
...
type CommunityLoaderByKey struct {
	...
	fetch func(keys []persist.CommunityKey) ([]coredb.Community, []error)
	...
	cache map[persist.CommunityKey]coredb.Community

	// typed cache functions
	//subscribers []func(coredb.Community)
	subscribers []communityLoaderByKeySubscriber
	...
	batch *communityLoaderByKeyBatch
...
type communityLoaderByKeyBatch struct {
	keys    []persist.CommunityKey
	data    []coredb.Community
	error   []error
	closing bool
	done    chan struct{}
}
...
// LoadThunk returns a function that when called will block waiting for a Community.
...
func (l *CommunityLoaderByKey) LoadThunk(key persist.CommunityKey) func() (coredb.Community, error) {
	...
	if !l.disableCaching {
		if it, ok := l.cache[key]; ok {
			l.mu.Unlock()
			return func() (coredb.Community, error) {
				return it, nil
			}
		}
	}
	if l.batch == nil {
		l.batch = &communityLoaderByKeyBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *CommunityLoaderByKey) LoadAll(keys []persist.CommunityKey) ([]coredb.Community, []error) {
	results := make([]func() (coredb.Community, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	communitys := make([]coredb.Community, len(keys))
	errors := make([]error, len(keys))
	for i, thunk := range results {
		communitys[i], errors[i] = thunk()
	}
	return communitys, errors
}

// LoadAllThunk returns a function that when called will block waiting for a Communitys.
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
...
func (l *CommunityLoaderByKey) unsafePrime(key persist.CommunityKey, value coredb.Community) bool {
	if l.disableCaching {
		return false
	}
	var found bool
...
func (l *CommunityLoaderByKey) unsafeSet(key persist.CommunityKey, value coredb.Community) {
	if l.cache == nil {
		l.cache = map[persist.CommunityKey]coredb.Community{}
	}
	l.cache[key] = value
}
...
func (b *communityLoaderByKeyBatch) keyIndex(l *CommunityLoaderByKey, key persist.CommunityKey) int {
...
func (b *communityLoaderByKeyBatch) startTimer(l *CommunityLoaderByKey) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *communityLoaderByKeyBatch) end(l *CommunityLoaderByKey) {
	b.data, b.error = l.fetch(b.keys)
	close(b.done)
}

type communityLoaderByKeySubscriber struct {
	cacheFunc func(coredb.Community)
	mutex     *sync.Mutex
}

func (l *CommunityLoaderByKey) publishToSubscribers(value coredb.Community) {
	// Lazy build our list of typed cache functions once
	l.once.Do(func() {
		for i, subscription := range *l.subscriptionRegistry {
			if typedFunc, ok := subscription.(*func(coredb.Community)); ok {
				// Don't invoke our own cache function
				if !l.ownsCacheFunc(typedFunc) {
					l.subscribers = append(l.subscribers, communityLoaderByKeySubscriber{cacheFunc: *typedFunc, mutex: (*l.mutexRegistry)[i]})
				}
			}
		}
	})
...
func (l *CommunityLoaderByKey) registerCacheFunc(cacheFunc interface{}, mutex *sync.Mutex) {
...
func (l *CommunityLoaderByKey) ownsCacheFunc(f *func(coredb.Community)) bool {
