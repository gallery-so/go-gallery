...
type TokenDefinitionByIDSettings interface {
...
// TokenDefinitionByIDCacheSubscriptions
type TokenDefinitionByIDCacheSubscriptions struct {
	// AutoCacheWithKey is a function that returns the persist.DBID cache key for a coredb.TokenDefinition.
	// If AutoCacheWithKey is not nil, this loader will automatically cache published results from other loaders
	// that return a coredb.TokenDefinition. Loaders that return pointers or slices of coredb.TokenDefinition
	// will be dereferenced/iterated automatically, invoking this function with the base coredb.TokenDefinition type.
	AutoCacheWithKey func(coredb.TokenDefinition) persist.DBID

	// AutoCacheWithKeys is a function that returns the []persist.DBID cache keys for a coredb.TokenDefinition.
	...
	// that return a coredb.TokenDefinition. Loaders that return pointers or slices of coredb.TokenDefinition
	// will be dereferenced/iterated automatically, invoking this function with the base coredb.TokenDefinition type.
	AutoCacheWithKeys func(coredb.TokenDefinition) []persist.DBID
...
func (l *TokenDefinitionByID) setContext(ctx context.Context) {
	l.ctx = ctx
}

func (l *TokenDefinitionByID) setWait(wait time.Duration) {
	l.wait = wait
}

func (l *TokenDefinitionByID) setMaxBatch(maxBatch int) {
	l.maxBatch = maxBatch
}

func (l *TokenDefinitionByID) setDisableCaching(disableCaching bool) {
	l.disableCaching = disableCaching
}

func (l *TokenDefinitionByID) setPublishResults(publishResults bool) {
	l.publishResults = publishResults
}

func (l *TokenDefinitionByID) setPreFetchHook(preFetchHook func(context.Context, string) context.Context) {
	l.preFetchHook = preFetchHook
}

func (l *TokenDefinitionByID) setPostFetchHook(postFetchHook func(context.Context, string)) {
	l.postFetchHook = postFetchHook
}

// NewTokenDefinitionByID creates a new TokenDefinitionByID with the given settings, functions, and options
func NewTokenDefinitionByID(
	settings TokenDefinitionByIDSettings, fetch func(ctx context.Context, keys []persist.DBID) ([]coredb.TokenDefinition, []error),
	funcs TokenDefinitionByIDCacheSubscriptions,
	opts ...func(interface {
	...
	loader.fetch = func(keys []persist.DBID) ([]coredb.TokenDefinition, []error) {
		ctx := loader.ctx

		// Allow the preFetchHook to modify and return a new context
		if loader.preFetchHook != nil {
			ctx = loader.preFetchHook(ctx, "TokenDefinitionByID")
		}

		results, errors := fetch(ctx, keys)

		if loader.postFetchHook != nil {
			loader.postFetchHook(ctx, "TokenDefinitionByID")
		}
	...
	if !loader.disableCaching {
		...
		if funcs.AutoCacheWithKey != nil {
			cacheFunc := func(t coredb.TokenDefinition) {
...
// TokenDefinitionByID batches and caches requests
type TokenDefinitionByID struct {
	// context passed to fetch functions
	ctx context.Context

	// this method provides the data for the loader
	fetch func(keys []persist.DBID) ([]coredb.TokenDefinition, []error)
	...
	cache map[persist.DBID]coredb.TokenDefinition

	// typed cache functions
	//subscribers []func(coredb.TokenDefinition)
	subscribers []tokenDefinitionByIDSubscriber
	...
	batch *tokenDefinitionByIDBatch
...
type tokenDefinitionByIDBatch struct {
	keys    []persist.DBID
	data    []coredb.TokenDefinition
	error   []error
	closing bool
	done    chan struct{}
}

// Load a TokenDefinition by key, batching and caching will be applied automatically
func (l *TokenDefinitionByID) Load(key persist.DBID) (coredb.TokenDefinition, error) {
	return l.LoadThunk(key)()
}

// LoadThunk returns a function that when called will block waiting for a TokenDefinition.
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *TokenDefinitionByID) LoadThunk(key persist.DBID) func() (coredb.TokenDefinition, error) {
	l.mu.Lock()
	if !l.disableCaching {
		if it, ok := l.cache[key]; ok {
			l.mu.Unlock()
			return func() (coredb.TokenDefinition, error) {
				return it, nil
			}
		}
	}
	if l.batch == nil {
		l.batch = &tokenDefinitionByIDBatch{done: make(chan struct{})}
	}
	batch := l.batch
	...
	return func() (coredb.TokenDefinition, error) {
		...
		var data coredb.TokenDefinition
...
func (l *TokenDefinitionByID) LoadAll(keys []persist.DBID) ([]coredb.TokenDefinition, []error) {
	results := make([]func() (coredb.TokenDefinition, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	tokenDefinitions := make([]coredb.TokenDefinition, len(keys))
	errors := make([]error, len(keys))
	for i, thunk := range results {
		tokenDefinitions[i], errors[i] = thunk()
	}
	return tokenDefinitions, errors
}

// LoadAllThunk returns a function that when called will block waiting for a TokenDefinitions.
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *TokenDefinitionByID) LoadAllThunk(keys []persist.DBID) func() ([]coredb.TokenDefinition, []error) {
	results := make([]func() (coredb.TokenDefinition, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *TokenDefinitionByID) Prime(key persist.DBID, value coredb.TokenDefinition) bool {
	if l.disableCaching {
		return false
	}
	l.mu.Lock()
...
func (l *TokenDefinitionByID) unsafePrime(key persist.DBID, value coredb.TokenDefinition) bool {
	if l.disableCaching {
		return false
	}
	var found bool
...
func (l *TokenDefinitionByID) Clear(key persist.DBID) {
	if l.disableCaching {
		return
	}
	l.mu.Lock()
...
func (l *TokenDefinitionByID) unsafeSet(key persist.DBID, value coredb.TokenDefinition) {
	if l.cache == nil {
		l.cache = map[persist.DBID]coredb.TokenDefinition{}
	}
	l.cache[key] = value
}
...
func (b *tokenDefinitionByIDBatch) keyIndex(l *TokenDefinitionByID, key persist.DBID) int {
...
func (b *tokenDefinitionByIDBatch) startTimer(l *TokenDefinitionByID) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *tokenDefinitionByIDBatch) end(l *TokenDefinitionByID) {
	b.data, b.error = l.fetch(b.keys)
	close(b.done)
}

type tokenDefinitionByIDSubscriber struct {
	cacheFunc func(coredb.TokenDefinition)
	mutex     *sync.Mutex
}

func (l *TokenDefinitionByID) publishToSubscribers(value coredb.TokenDefinition) {
	// Lazy build our list of typed cache functions once
	l.once.Do(func() {
		for i, subscription := range *l.subscriptionRegistry {
			if typedFunc, ok := subscription.(*func(coredb.TokenDefinition)); ok {
				// Don't invoke our own cache function
				if !l.ownsCacheFunc(typedFunc) {
					l.subscribers = append(l.subscribers, tokenDefinitionByIDSubscriber{cacheFunc: *typedFunc, mutex: (*l.mutexRegistry)[i]})
				}
			}
		}
	})
...
func (l *TokenDefinitionByID) registerCacheFunc(cacheFunc interface{}, mutex *sync.Mutex) {
...
func (l *TokenDefinitionByID) ownsCacheFunc(f *func(coredb.TokenDefinition)) bool {
