...
type UserLoaderByIDConfig struct {
	...
	Fetch func(keys []persist.DBID) ([]persist.User, []error)
...
type UserLoaderByID struct {
	...
	cache map[persist.DBID]persist.User
...
func (l *UserLoaderByID) Load(key persist.DBID) (persist.User, error) {
...
func (l *UserLoaderByID) LoadThunk(key persist.DBID) func() (persist.User, error) {
	l.mu.Lock()
...
func (l *UserLoaderByID) LoadAll(keys []persist.DBID) ([]persist.User, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *UserLoaderByID) LoadAllThunk(keys []persist.DBID) func() ([]persist.User, []error) {
	results := make([]func() (persist.User, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *UserLoaderByID) Prime(key persist.DBID, value persist.User) bool {
	l.mu.Lock()
	var found bool
...
func (l *UserLoaderByID) unsafeSet(key persist.DBID, value persist.User) {
	if l.cache == nil {
		l.cache = map[persist.DBID]persist.User{}
	}
	l.cache[key] = value
}
