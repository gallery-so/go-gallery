...
type AdmireByActorIDAndTokenSettings interface {
...
// AdmireByActorIDAndTokenCacheSubscriptions
type AdmireByActorIDAndTokenCacheSubscriptions struct {
...
func (l *AdmireByActorIDAndToken) setContext(ctx context.Context) {
	l.ctx = ctx
}

func (l *AdmireByActorIDAndToken) setWait(wait time.Duration) {
	l.wait = wait
}

func (l *AdmireByActorIDAndToken) setMaxBatch(maxBatch int) {
	l.maxBatch = maxBatch
}

func (l *AdmireByActorIDAndToken) setDisableCaching(disableCaching bool) {
	l.disableCaching = disableCaching
}

func (l *AdmireByActorIDAndToken) setPublishResults(publishResults bool) {
	l.publishResults = publishResults
}

func (l *AdmireByActorIDAndToken) setPreFetchHook(preFetchHook func(context.Context, string) context.Context) {
	l.preFetchHook = preFetchHook
}

func (l *AdmireByActorIDAndToken) setPostFetchHook(postFetchHook func(context.Context, string)) {
	l.postFetchHook = postFetchHook
}

// NewAdmireByActorIDAndToken creates a new AdmireByActorIDAndToken with the given settings, functions, and options
func NewAdmireByActorIDAndToken(
	settings AdmireByActorIDAndTokenSettings, fetch func(ctx context.Context, keys []coredb.GetAdmireByActorIDAndTokenIDParams) ([]coredb.Admire, []error),
	funcs AdmireByActorIDAndTokenCacheSubscriptions,
	opts ...func(interface {
...
) *AdmireByActorIDAndToken {
	loader := &AdmireByActorIDAndToken{
	...
	loader.fetch = func(keys []coredb.GetAdmireByActorIDAndTokenIDParams) ([]coredb.Admire, []error) {
		...
		if loader.preFetchHook != nil {
			ctx = loader.preFetchHook(ctx, "AdmireByActorIDAndToken")
		}

		results, errors := fetch(ctx, keys)

		if loader.postFetchHook != nil {
			loader.postFetchHook(ctx, "AdmireByActorIDAndToken")
		}
...
// AdmireByActorIDAndToken batches and caches requests
type AdmireByActorIDAndToken struct {
	...
	subscribers []admireByActorIDAndTokenSubscriber
	...
	batch *admireByActorIDAndTokenBatch
...
type admireByActorIDAndTokenBatch struct {
...
func (l *AdmireByActorIDAndToken) Load(key coredb.GetAdmireByActorIDAndTokenIDParams) (coredb.Admire, error) {
...
func (l *AdmireByActorIDAndToken) LoadThunk(key coredb.GetAdmireByActorIDAndTokenIDParams) func() (coredb.Admire, error) {
	l.mu.Lock()
	if !l.disableCaching {
	...
	if l.batch == nil {
		l.batch = &admireByActorIDAndTokenBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *AdmireByActorIDAndToken) LoadAll(keys []coredb.GetAdmireByActorIDAndTokenIDParams) ([]coredb.Admire, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *AdmireByActorIDAndToken) LoadAllThunk(keys []coredb.GetAdmireByActorIDAndTokenIDParams) func() ([]coredb.Admire, []error) {
	results := make([]func() (coredb.Admire, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *AdmireByActorIDAndToken) Prime(key coredb.GetAdmireByActorIDAndTokenIDParams, value coredb.Admire) bool {
	if l.disableCaching {
		return false
	}
	l.mu.Lock()
...
func (l *AdmireByActorIDAndToken) unsafePrime(key coredb.GetAdmireByActorIDAndTokenIDParams, value coredb.Admire) bool {
	if l.disableCaching {
		return false
	}
	var found bool
...
func (l *AdmireByActorIDAndToken) Clear(key coredb.GetAdmireByActorIDAndTokenIDParams) {
	if l.disableCaching {
		return
	}
	l.mu.Lock()
...
func (l *AdmireByActorIDAndToken) unsafeSet(key coredb.GetAdmireByActorIDAndTokenIDParams, value coredb.Admire) {
	if l.cache == nil {
...
func (b *admireByActorIDAndTokenBatch) keyIndex(l *AdmireByActorIDAndToken, key coredb.GetAdmireByActorIDAndTokenIDParams) int {
...
func (b *admireByActorIDAndTokenBatch) startTimer(l *AdmireByActorIDAndToken) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *admireByActorIDAndTokenBatch) end(l *AdmireByActorIDAndToken) {
	b.data, b.error = l.fetch(b.keys)
	close(b.done)
}

type admireByActorIDAndTokenSubscriber struct {
	cacheFunc func(coredb.Admire)
	mutex     *sync.Mutex
}

func (l *AdmireByActorIDAndToken) publishToSubscribers(value coredb.Admire) {
		...
		for i, subscription := range *l.subscriptionRegistry {
			if typedFunc, ok := subscription.(*func(coredb.Admire)); ok {
				...
				if !l.ownsCacheFunc(typedFunc) {
					l.subscribers = append(l.subscribers, admireByActorIDAndTokenSubscriber{cacheFunc: *typedFunc, mutex: (*l.mutexRegistry)[i]})
				}
			}
		}
	})
...
func (l *AdmireByActorIDAndToken) registerCacheFunc(cacheFunc interface{}, mutex *sync.Mutex) {
...
func (l *AdmireByActorIDAndToken) ownsCacheFunc(f *func(coredb.Admire)) bool {
