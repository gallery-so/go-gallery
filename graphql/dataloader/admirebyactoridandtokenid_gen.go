...
type AdmireByActorIDAndTokenIDSettings interface {
...
// AdmireByActorIDAndTokenIDCacheSubscriptions
type AdmireByActorIDAndTokenIDCacheSubscriptions struct {
...
func (l *AdmireByActorIDAndTokenID) setContext(ctx context.Context) {
	l.ctx = ctx
}

func (l *AdmireByActorIDAndTokenID) setWait(wait time.Duration) {
	l.wait = wait
}

func (l *AdmireByActorIDAndTokenID) setMaxBatch(maxBatch int) {
	l.maxBatch = maxBatch
}

func (l *AdmireByActorIDAndTokenID) setDisableCaching(disableCaching bool) {
	l.disableCaching = disableCaching
}

func (l *AdmireByActorIDAndTokenID) setPublishResults(publishResults bool) {
	l.publishResults = publishResults
}

func (l *AdmireByActorIDAndTokenID) setPreFetchHook(preFetchHook func(context.Context, string) context.Context) {
	l.preFetchHook = preFetchHook
}

func (l *AdmireByActorIDAndTokenID) setPostFetchHook(postFetchHook func(context.Context, string)) {
	l.postFetchHook = postFetchHook
}

// NewAdmireByActorIDAndTokenID creates a new AdmireByActorIDAndTokenID with the given settings, functions, and options
func NewAdmireByActorIDAndTokenID(
	settings AdmireByActorIDAndTokenIDSettings, fetch func(ctx context.Context, keys []coredb.GetAdmireByActorIDAndTokenIDParams) ([]coredb.Admire, []error),
	funcs AdmireByActorIDAndTokenIDCacheSubscriptions,
	opts ...func(interface {
...
) *AdmireByActorIDAndTokenID {
	loader := &AdmireByActorIDAndTokenID{
	...
	loader.fetch = func(keys []coredb.GetAdmireByActorIDAndTokenIDParams) ([]coredb.Admire, []error) {
		...
		if loader.preFetchHook != nil {
			ctx = loader.preFetchHook(ctx, "AdmireByActorIDAndTokenID")
		}

		results, errors := fetch(ctx, keys)

		if loader.postFetchHook != nil {
			loader.postFetchHook(ctx, "AdmireByActorIDAndTokenID")
		}
...
// AdmireByActorIDAndTokenID batches and caches requests
type AdmireByActorIDAndTokenID struct {
	...
	subscribers []admireByActorIDAndTokenIDSubscriber
	...
	batch *admireByActorIDAndTokenIDBatch
...
type admireByActorIDAndTokenIDBatch struct {
...
func (l *AdmireByActorIDAndTokenID) Load(key coredb.GetAdmireByActorIDAndTokenIDParams) (coredb.Admire, error) {
...
func (l *AdmireByActorIDAndTokenID) LoadThunk(key coredb.GetAdmireByActorIDAndTokenIDParams) func() (coredb.Admire, error) {
	l.mu.Lock()
	if !l.disableCaching {
	...
	if l.batch == nil {
		l.batch = &admireByActorIDAndTokenIDBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *AdmireByActorIDAndTokenID) LoadAll(keys []coredb.GetAdmireByActorIDAndTokenIDParams) ([]coredb.Admire, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *AdmireByActorIDAndTokenID) LoadAllThunk(keys []coredb.GetAdmireByActorIDAndTokenIDParams) func() ([]coredb.Admire, []error) {
	results := make([]func() (coredb.Admire, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *AdmireByActorIDAndTokenID) Prime(key coredb.GetAdmireByActorIDAndTokenIDParams, value coredb.Admire) bool {
	if l.disableCaching {
		return false
	}
	l.mu.Lock()
...
func (l *AdmireByActorIDAndTokenID) unsafePrime(key coredb.GetAdmireByActorIDAndTokenIDParams, value coredb.Admire) bool {
	if l.disableCaching {
		return false
	}
	var found bool
...
func (l *AdmireByActorIDAndTokenID) Clear(key coredb.GetAdmireByActorIDAndTokenIDParams) {
	if l.disableCaching {
		return
	}
	l.mu.Lock()
...
func (l *AdmireByActorIDAndTokenID) unsafeSet(key coredb.GetAdmireByActorIDAndTokenIDParams, value coredb.Admire) {
	if l.cache == nil {
...
func (b *admireByActorIDAndTokenIDBatch) keyIndex(l *AdmireByActorIDAndTokenID, key coredb.GetAdmireByActorIDAndTokenIDParams) int {
...
func (b *admireByActorIDAndTokenIDBatch) startTimer(l *AdmireByActorIDAndTokenID) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *admireByActorIDAndTokenIDBatch) end(l *AdmireByActorIDAndTokenID) {
	b.data, b.error = l.fetch(b.keys)
	close(b.done)
}

type admireByActorIDAndTokenIDSubscriber struct {
	cacheFunc func(coredb.Admire)
	mutex     *sync.Mutex
}

func (l *AdmireByActorIDAndTokenID) publishToSubscribers(value coredb.Admire) {
		...
		for i, subscription := range *l.subscriptionRegistry {
			if typedFunc, ok := subscription.(*func(coredb.Admire)); ok {
				...
				if !l.ownsCacheFunc(typedFunc) {
					l.subscribers = append(l.subscribers, admireByActorIDAndTokenIDSubscriber{cacheFunc: *typedFunc, mutex: (*l.mutexRegistry)[i]})
				}
			}
		}
	})
...
func (l *AdmireByActorIDAndTokenID) registerCacheFunc(cacheFunc interface{}, mutex *sync.Mutex) {
...
func (l *AdmireByActorIDAndTokenID) ownsCacheFunc(f *func(coredb.Admire)) bool {
