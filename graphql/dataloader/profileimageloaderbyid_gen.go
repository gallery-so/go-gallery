...
func (l *ProfileImageLoaderByID) setContext(ctx context.Context) {
	l.ctx = ctx
}

func (l *ProfileImageLoaderByID) setWait(wait time.Duration) {
	l.wait = wait
}

func (l *ProfileImageLoaderByID) setMaxBatch(maxBatch int) {
	l.maxBatch = maxBatch
}

func (l *ProfileImageLoaderByID) setDisableCaching(disableCaching bool) {
	l.disableCaching = disableCaching
}

func (l *ProfileImageLoaderByID) setPublishResults(publishResults bool) {
	l.publishResults = publishResults
}

func (l *ProfileImageLoaderByID) setPreFetchHook(preFetchHook func(context.Context, string) context.Context) {
	l.preFetchHook = preFetchHook
}

func (l *ProfileImageLoaderByID) setPostFetchHook(postFetchHook func(context.Context, string)) {
...
func NewProfileImageLoaderByID(
...
) *ProfileImageLoaderByID {
	loader := &ProfileImageLoaderByID{
	...
	loader.fetch = func(keys []coredb.GetProfileImageByIDParams) ([]coredb.ProfileImage, []error) {
		...
		if loader.preFetchHook != nil {
			ctx = loader.preFetchHook(ctx, "ProfileImageLoaderByID")
		}

		results, errors := fetch(ctx, keys)

		if loader.postFetchHook != nil {
			loader.postFetchHook(ctx, "ProfileImageLoaderByID")
		}
	...
	if !loader.disableCaching {
		...
		if funcs.AutoCacheWithKey != nil {
			cacheFunc := func(t coredb.ProfileImage) {
...
// ProfileImageLoaderByID batches and caches requests
type ProfileImageLoaderByID struct {
	...
	subscribers []profileImageLoaderByIDSubscriber
	...
	batch *profileImageLoaderByIDBatch
...
func (l *ProfileImageLoaderByID) LoadThunk(key coredb.GetProfileImageByIDParams) func() (coredb.ProfileImage, error) {
	...
	if !l.disableCaching {
		if it, ok := l.cache[key]; ok {
			l.mu.Unlock()
			return func() (coredb.ProfileImage, error) {
				return it, nil
			}
		}
	}
	if l.batch == nil {
		l.batch = &profileImageLoaderByIDBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *ProfileImageLoaderByID) LoadAll(keys []coredb.GetProfileImageByIDParams) ([]coredb.ProfileImage, []error) {
	...
	profileImages := make([]coredb.ProfileImage, len(keys))
	errors := make([]error, len(keys))
	for i, thunk := range results {
		profileImages[i], errors[i] = thunk()
	}
...
func (b *profileImageLoaderByIDBatch) startTimer(l *ProfileImageLoaderByID) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *profileImageLoaderByIDBatch) end(l *ProfileImageLoaderByID) {
	b.data, b.error = l.fetch(b.keys)
	close(b.done)
}

type profileImageLoaderByIDSubscriber struct {
	cacheFunc func(coredb.ProfileImage)
	mutex     *sync.Mutex
}

func (l *ProfileImageLoaderByID) publishToSubscribers(value coredb.ProfileImage) {
	// Lazy build our list of typed cache functions once
	l.once.Do(func() {
		for i, subscription := range *l.subscriptionRegistry {
			if typedFunc, ok := subscription.(*func(coredb.ProfileImage)); ok {
				// Don't invoke our own cache function
				if !l.ownsCacheFunc(typedFunc) {
					l.subscribers = append(l.subscribers, profileImageLoaderByIDSubscriber{cacheFunc: *typedFunc, mutex: (*l.mutexRegistry)[i]})
				}
			}
		}
	})
...
func (l *ProfileImageLoaderByID) registerCacheFunc(cacheFunc interface{}, mutex *sync.Mutex) {
...
func (l *ProfileImageLoaderByID) ownsCacheFunc(f *func(coredb.ProfileImage)) bool {
