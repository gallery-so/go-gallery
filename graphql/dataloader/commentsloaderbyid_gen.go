...
type CommentsLoaderByIDSettings interface {
...
func (l *CommentsLoaderByID) setContext(ctx context.Context) {
	l.ctx = ctx
}

func (l *CommentsLoaderByID) setWait(wait time.Duration) {
	l.wait = wait
}

func (l *CommentsLoaderByID) setMaxBatch(maxBatch int) {
	l.maxBatch = maxBatch
}

func (l *CommentsLoaderByID) setDisableCaching(disableCaching bool) {
	l.disableCaching = disableCaching
}

func (l *CommentsLoaderByID) setPublishResults(publishResults bool) {
	l.publishResults = publishResults
}

func (l *CommentsLoaderByID) setPreFetchHook(preFetchHook func(context.Context, string) context.Context) {
	l.preFetchHook = preFetchHook
}

func (l *CommentsLoaderByID) setPostFetchHook(postFetchHook func(context.Context, string)) {
	l.postFetchHook = postFetchHook
}

// NewCommentsLoaderByID creates a new CommentsLoaderByID with the given settings, functions, and options
func NewCommentsLoaderByID(
	settings CommentsLoaderByIDSettings, fetch func(ctx context.Context, keys []persist.DBID) ([][]coredb.Comment, []error),
	opts ...func(interface {
	...
	loader.fetch = func(keys []persist.DBID) ([][]coredb.Comment, []error) {
		...
		if loader.preFetchHook != nil {
			ctx = loader.preFetchHook(ctx, "CommentsLoaderByID")
		}

		results, errors := fetch(ctx, keys)

		if loader.postFetchHook != nil {
			loader.postFetchHook(ctx, "CommentsLoaderByID")
		}
...
type CommentsLoaderByID struct {
	...
	subscribers []commentsLoaderByIDSubscriber
...
func (l *CommentsLoaderByID) LoadThunk(key persist.DBID) func() ([]coredb.Comment, error) {
	...
	if l.batch == nil {
		l.batch = &commentsLoaderByIDBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *CommentsLoaderByID) Clear(key persist.DBID) {
	if l.disableCaching {
		return
	}
	l.mu.Lock()
...
func (b *commentsLoaderByIDBatch) keyIndex(l *CommentsLoaderByID, key persist.DBID) int {
...
func (b *commentsLoaderByIDBatch) startTimer(l *CommentsLoaderByID) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *commentsLoaderByIDBatch) end(l *CommentsLoaderByID) {
	b.data, b.error = l.fetch(b.keys)
	close(b.done)
}

type commentsLoaderByIDSubscriber struct {
	cacheFunc func(coredb.Comment)
	mutex     *sync.Mutex
}

func (l *CommentsLoaderByID) publishToSubscribers(value []coredb.Comment) {
		...
		for i, subscription := range *l.subscriptionRegistry {
			if typedFunc, ok := subscription.(*func(coredb.Comment)); ok {
				...
				if !l.ownsCacheFunc(typedFunc) {
					l.subscribers = append(l.subscribers, commentsLoaderByIDSubscriber{cacheFunc: *typedFunc, mutex: (*l.mutexRegistry)[i]})
				}
			}
		}
	})
...
func (l *CommentsLoaderByID) registerCacheFunc(cacheFunc interface{}, mutex *sync.Mutex) {
...
func (l *CommentsLoaderByID) ownsCacheFunc(f *func(coredb.Comment)) bool {
