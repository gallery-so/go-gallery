...
type MentionsLoaderByIDSettings interface {
...
func (l *MentionsLoaderByID) setContext(ctx context.Context) {
	l.ctx = ctx
}

func (l *MentionsLoaderByID) setWait(wait time.Duration) {
	l.wait = wait
}

func (l *MentionsLoaderByID) setMaxBatch(maxBatch int) {
	l.maxBatch = maxBatch
}

func (l *MentionsLoaderByID) setDisableCaching(disableCaching bool) {
	l.disableCaching = disableCaching
}

func (l *MentionsLoaderByID) setPublishResults(publishResults bool) {
	l.publishResults = publishResults
}

func (l *MentionsLoaderByID) setPreFetchHook(preFetchHook func(context.Context, string) context.Context) {
	l.preFetchHook = preFetchHook
}

func (l *MentionsLoaderByID) setPostFetchHook(postFetchHook func(context.Context, string)) {
	l.postFetchHook = postFetchHook
}

// NewMentionsLoaderByID creates a new MentionsLoaderByID with the given settings, functions, and options
func NewMentionsLoaderByID(
	settings MentionsLoaderByIDSettings, fetch func(ctx context.Context, keys []persist.DBID) ([][]coredb.Mention, []error),
	opts ...func(interface {
	...
	loader.fetch = func(keys []persist.DBID) ([][]coredb.Mention, []error) {
		ctx := loader.ctx

		// Allow the preFetchHook to modify and return a new context
		if loader.preFetchHook != nil {
			ctx = loader.preFetchHook(ctx, "MentionsLoaderByID")
		}

		results, errors := fetch(ctx, keys)

		if loader.postFetchHook != nil {
			loader.postFetchHook(ctx, "MentionsLoaderByID")
		}
...
// MentionsLoaderByID batches and caches requests
type MentionsLoaderByID struct {
	// context passed to fetch functions
	ctx context.Context

	// this method provides the data for the loader
	fetch func(keys []persist.DBID) ([][]coredb.Mention, []error)
	...
	cache map[persist.DBID][]coredb.Mention

	// typed cache functions
	//subscribers []func([]coredb.Mention)
	subscribers []mentionsLoaderByIDSubscriber
...
type mentionsLoaderByIDBatch struct {
	keys    []persist.DBID
	data    [][]coredb.Mention
	error   []error
	closing bool
...
// Load a Mention by key, batching and caching will be applied automatically
func (l *MentionsLoaderByID) Load(key persist.DBID) ([]coredb.Mention, error) {
	return l.LoadThunk(key)()
}

// LoadThunk returns a function that when called will block waiting for a Mention.
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *MentionsLoaderByID) LoadThunk(key persist.DBID) func() ([]coredb.Mention, error) {
	l.mu.Lock()
	if !l.disableCaching {
		if it, ok := l.cache[key]; ok {
			l.mu.Unlock()
			return func() ([]coredb.Mention, error) {
				return it, nil
			}
		}
	}
	if l.batch == nil {
		l.batch = &mentionsLoaderByIDBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *MentionsLoaderByID) LoadAll(keys []persist.DBID) ([][]coredb.Mention, []error) {
	results := make([]func() ([]coredb.Mention, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	mentions := make([][]coredb.Mention, len(keys))
	errors := make([]error, len(keys))
	for i, thunk := range results {
		mentions[i], errors[i] = thunk()
	}
	return mentions, errors
}

// LoadAllThunk returns a function that when called will block waiting for a Mentions.
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *MentionsLoaderByID) LoadAllThunk(keys []persist.DBID) func() ([][]coredb.Mention, []error) {
	results := make([]func() ([]coredb.Mention, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *MentionsLoaderByID) Prime(key persist.DBID, value []coredb.Mention) bool {
	if l.disableCaching {
		return false
	}
	l.mu.Lock()
	...
	if _, found = l.cache[key]; !found {
		...
		cpy := make([]coredb.Mention, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
...
func (l *MentionsLoaderByID) Clear(key persist.DBID) {
	if l.disableCaching {
		return
	}
	l.mu.Lock()
...
func (l *MentionsLoaderByID) unsafeSet(key persist.DBID, value []coredb.Mention) {
	if l.cache == nil {
		l.cache = map[persist.DBID][]coredb.Mention{}
	}
	l.cache[key] = value
}
...
func (b *mentionsLoaderByIDBatch) keyIndex(l *MentionsLoaderByID, key persist.DBID) int {
...
func (b *mentionsLoaderByIDBatch) startTimer(l *MentionsLoaderByID) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *mentionsLoaderByIDBatch) end(l *MentionsLoaderByID) {
	b.data, b.error = l.fetch(b.keys)
	close(b.done)
}

type mentionsLoaderByIDSubscriber struct {
	cacheFunc func(coredb.Mention)
	mutex     *sync.Mutex
}

func (l *MentionsLoaderByID) publishToSubscribers(value []coredb.Mention) {
	// Lazy build our list of typed cache functions once
	l.once.Do(func() {
		for i, subscription := range *l.subscriptionRegistry {
			if typedFunc, ok := subscription.(*func(coredb.Mention)); ok {
				// Don't invoke our own cache function
				if !l.ownsCacheFunc(typedFunc) {
					l.subscribers = append(l.subscribers, mentionsLoaderByIDSubscriber{cacheFunc: *typedFunc, mutex: (*l.mutexRegistry)[i]})
				}
			}
		}
	})
...
func (l *MentionsLoaderByID) registerCacheFunc(cacheFunc interface{}, mutex *sync.Mutex) {
...
func (l *MentionsLoaderByID) ownsCacheFunc(f *func(coredb.Mention)) bool {
