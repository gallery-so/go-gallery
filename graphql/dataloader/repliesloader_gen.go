...
type RepliesLoaderSettings interface {
...
func (l *RepliesLoader) setContext(ctx context.Context) {
	l.ctx = ctx
}

func (l *RepliesLoader) setWait(wait time.Duration) {
	l.wait = wait
}

func (l *RepliesLoader) setMaxBatch(maxBatch int) {
	l.maxBatch = maxBatch
}

func (l *RepliesLoader) setDisableCaching(disableCaching bool) {
	l.disableCaching = disableCaching
}

func (l *RepliesLoader) setPublishResults(publishResults bool) {
	l.publishResults = publishResults
}

func (l *RepliesLoader) setPreFetchHook(preFetchHook func(context.Context, string) context.Context) {
	l.preFetchHook = preFetchHook
}

func (l *RepliesLoader) setPostFetchHook(postFetchHook func(context.Context, string)) {
	l.postFetchHook = postFetchHook
}

// NewRepliesLoader creates a new RepliesLoader with the given settings, functions, and options
func NewRepliesLoader(
	settings RepliesLoaderSettings, fetch func(ctx context.Context, keys []coredb.PaginateRepliesByCommentIDBatchParams) ([][]coredb.Comment, []error),
	opts ...func(interface {
	...
	loader.fetch = func(keys []coredb.PaginateRepliesByCommentIDBatchParams) ([][]coredb.Comment, []error) {
		ctx := loader.ctx

		// Allow the preFetchHook to modify and return a new context
		if loader.preFetchHook != nil {
			ctx = loader.preFetchHook(ctx, "RepliesLoader")
		}

		results, errors := fetch(ctx, keys)

		if loader.postFetchHook != nil {
			loader.postFetchHook(ctx, "RepliesLoader")
		}
...
// RepliesLoader batches and caches requests
type RepliesLoader struct {
	// context passed to fetch functions
	ctx context.Context

	// this method provides the data for the loader
	fetch func(keys []coredb.PaginateRepliesByCommentIDBatchParams) ([][]coredb.Comment, []error)
	...
	cache map[coredb.PaginateRepliesByCommentIDBatchParams][]coredb.Comment

	// typed cache functions
	//subscribers []func([]coredb.Comment)
	subscribers []repliesLoaderSubscriber
...
type repliesLoaderBatch struct {
	keys    []coredb.PaginateRepliesByCommentIDBatchParams
	data    [][]coredb.Comment
	error   []error
	closing bool
	done    chan struct{}
...
func (l *RepliesLoader) Load(key coredb.PaginateRepliesByCommentIDBatchParams) ([]coredb.Comment, error) {
...
func (l *RepliesLoader) LoadThunk(key coredb.PaginateRepliesByCommentIDBatchParams) func() ([]coredb.Comment, error) {
	l.mu.Lock()
	if !l.disableCaching {
	...
	if l.batch == nil {
		l.batch = &repliesLoaderBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *RepliesLoader) LoadAll(keys []coredb.PaginateRepliesByCommentIDBatchParams) ([][]coredb.Comment, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *RepliesLoader) LoadAllThunk(keys []coredb.PaginateRepliesByCommentIDBatchParams) func() ([][]coredb.Comment, []error) {
	results := make([]func() ([]coredb.Comment, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *RepliesLoader) Prime(key coredb.PaginateRepliesByCommentIDBatchParams, value []coredb.Comment) bool {
	if l.disableCaching {
		return false
	}
	l.mu.Lock()
...
func (l *RepliesLoader) Clear(key coredb.PaginateRepliesByCommentIDBatchParams) {
	if l.disableCaching {
		return
	}
	l.mu.Lock()
...
func (l *RepliesLoader) unsafeSet(key coredb.PaginateRepliesByCommentIDBatchParams, value []coredb.Comment) {
	if l.cache == nil {
		l.cache = map[coredb.PaginateRepliesByCommentIDBatchParams][]coredb.Comment{}
	}
	l.cache[key] = value
}
...
func (b *repliesLoaderBatch) keyIndex(l *RepliesLoader, key coredb.PaginateRepliesByCommentIDBatchParams) int {
...
func (b *repliesLoaderBatch) startTimer(l *RepliesLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *repliesLoaderBatch) end(l *RepliesLoader) {
	b.data, b.error = l.fetch(b.keys)
	close(b.done)
}

type repliesLoaderSubscriber struct {
	cacheFunc func(coredb.Comment)
	mutex     *sync.Mutex
}

func (l *RepliesLoader) publishToSubscribers(value []coredb.Comment) {
		...
		for i, subscription := range *l.subscriptionRegistry {
			if typedFunc, ok := subscription.(*func(coredb.Comment)); ok {
				...
				if !l.ownsCacheFunc(typedFunc) {
					l.subscribers = append(l.subscribers, repliesLoaderSubscriber{cacheFunc: *typedFunc, mutex: (*l.mutexRegistry)[i]})
				}
			}
		}
	})
...
func (l *RepliesLoader) registerCacheFunc(cacheFunc interface{}, mutex *sync.Mutex) {
...
func (l *RepliesLoader) ownsCacheFunc(f *func(coredb.Comment)) bool {
