...
type AdmireLoaderByActorAndTokenSettings interface {
...
// AdmireLoaderByActorAndTokenCacheSubscriptions
type AdmireLoaderByActorAndTokenCacheSubscriptions struct {
	// AutoCacheWithKey is a function that returns the coredb.GetAdmireByActorIDAndTokenIDParams cache key for a coredb.Admire.
	...
	AutoCacheWithKey func(coredb.Admire) coredb.GetAdmireByActorIDAndTokenIDParams

	// AutoCacheWithKeys is a function that returns the []coredb.GetAdmireByActorIDAndTokenIDParams cache keys for a coredb.Admire.
	...
	// that return a coredb.Admire. Loaders that return pointers or slices of coredb.Admire
	// will be dereferenced/iterated automatically, invoking this function with the base coredb.Admire type.
	AutoCacheWithKeys func(coredb.Admire) []coredb.GetAdmireByActorIDAndTokenIDParams
...
func (l *AdmireLoaderByActorAndToken) setContext(ctx context.Context) {
	l.ctx = ctx
}

func (l *AdmireLoaderByActorAndToken) setWait(wait time.Duration) {
	l.wait = wait
}

func (l *AdmireLoaderByActorAndToken) setMaxBatch(maxBatch int) {
	l.maxBatch = maxBatch
}

func (l *AdmireLoaderByActorAndToken) setDisableCaching(disableCaching bool) {
	l.disableCaching = disableCaching
}

func (l *AdmireLoaderByActorAndToken) setPublishResults(publishResults bool) {
	l.publishResults = publishResults
}

func (l *AdmireLoaderByActorAndToken) setPreFetchHook(preFetchHook func(context.Context, string) context.Context) {
	l.preFetchHook = preFetchHook
}

func (l *AdmireLoaderByActorAndToken) setPostFetchHook(postFetchHook func(context.Context, string)) {
	l.postFetchHook = postFetchHook
}

// NewAdmireLoaderByActorAndToken creates a new AdmireLoaderByActorAndToken with the given settings, functions, and options
func NewAdmireLoaderByActorAndToken(
	settings AdmireLoaderByActorAndTokenSettings, fetch func(ctx context.Context, keys []coredb.GetAdmireByActorIDAndTokenIDParams) ([]coredb.Admire, []error),
	funcs AdmireLoaderByActorAndTokenCacheSubscriptions,
	opts ...func(interface {
...
) *AdmireLoaderByActorAndToken {
	loader := &AdmireLoaderByActorAndToken{
	...
	loader.fetch = func(keys []coredb.GetAdmireByActorIDAndTokenIDParams) ([]coredb.Admire, []error) {
		ctx := loader.ctx

		// Allow the preFetchHook to modify and return a new context
		if loader.preFetchHook != nil {
			ctx = loader.preFetchHook(ctx, "AdmireLoaderByActorAndToken")
		}

		results, errors := fetch(ctx, keys)

		if loader.postFetchHook != nil {
			loader.postFetchHook(ctx, "AdmireLoaderByActorAndToken")
		}
...
// AdmireLoaderByActorAndToken batches and caches requests
type AdmireLoaderByActorAndToken struct {
	// context passed to fetch functions
	ctx context.Context

	// this method provides the data for the loader
	fetch func(keys []coredb.GetAdmireByActorIDAndTokenIDParams) ([]coredb.Admire, []error)
	...
	cache map[coredb.GetAdmireByActorIDAndTokenIDParams]coredb.Admire

	// typed cache functions
	//subscribers []func(coredb.Admire)
	subscribers []admireLoaderByActorAndTokenSubscriber
	...
	batch *admireLoaderByActorAndTokenBatch
...
type admireLoaderByActorAndTokenBatch struct {
	keys    []coredb.GetAdmireByActorIDAndTokenIDParams
	data    []coredb.Admire
	error   []error
	closing bool
	done    chan struct{}
...
func (l *AdmireLoaderByActorAndToken) Load(key coredb.GetAdmireByActorIDAndTokenIDParams) (coredb.Admire, error) {
...
func (l *AdmireLoaderByActorAndToken) LoadThunk(key coredb.GetAdmireByActorIDAndTokenIDParams) func() (coredb.Admire, error) {
	l.mu.Lock()
	if !l.disableCaching {
	...
	if l.batch == nil {
		l.batch = &admireLoaderByActorAndTokenBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *AdmireLoaderByActorAndToken) LoadAll(keys []coredb.GetAdmireByActorIDAndTokenIDParams) ([]coredb.Admire, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *AdmireLoaderByActorAndToken) LoadAllThunk(keys []coredb.GetAdmireByActorIDAndTokenIDParams) func() ([]coredb.Admire, []error) {
	results := make([]func() (coredb.Admire, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *AdmireLoaderByActorAndToken) Prime(key coredb.GetAdmireByActorIDAndTokenIDParams, value coredb.Admire) bool {
	if l.disableCaching {
		return false
	}
	l.mu.Lock()
...
func (l *AdmireLoaderByActorAndToken) unsafePrime(key coredb.GetAdmireByActorIDAndTokenIDParams, value coredb.Admire) bool {
	if l.disableCaching {
		return false
	}
	var found bool
...
func (l *AdmireLoaderByActorAndToken) Clear(key coredb.GetAdmireByActorIDAndTokenIDParams) {
	if l.disableCaching {
		return
	}
	l.mu.Lock()
...
func (l *AdmireLoaderByActorAndToken) unsafeSet(key coredb.GetAdmireByActorIDAndTokenIDParams, value coredb.Admire) {
	if l.cache == nil {
		l.cache = map[coredb.GetAdmireByActorIDAndTokenIDParams]coredb.Admire{}
	}
	l.cache[key] = value
}
...
func (b *admireLoaderByActorAndTokenBatch) keyIndex(l *AdmireLoaderByActorAndToken, key coredb.GetAdmireByActorIDAndTokenIDParams) int {
...
func (b *admireLoaderByActorAndTokenBatch) startTimer(l *AdmireLoaderByActorAndToken) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *admireLoaderByActorAndTokenBatch) end(l *AdmireLoaderByActorAndToken) {
	b.data, b.error = l.fetch(b.keys)
	close(b.done)
}

type admireLoaderByActorAndTokenSubscriber struct {
	cacheFunc func(coredb.Admire)
	mutex     *sync.Mutex
}

func (l *AdmireLoaderByActorAndToken) publishToSubscribers(value coredb.Admire) {
		...
		for i, subscription := range *l.subscriptionRegistry {
			if typedFunc, ok := subscription.(*func(coredb.Admire)); ok {
				...
				if !l.ownsCacheFunc(typedFunc) {
					l.subscribers = append(l.subscribers, admireLoaderByActorAndTokenSubscriber{cacheFunc: *typedFunc, mutex: (*l.mutexRegistry)[i]})
				}
			}
		}
	})
...
func (l *AdmireLoaderByActorAndToken) registerCacheFunc(cacheFunc interface{}, mutex *sync.Mutex) {
...
func (l *AdmireLoaderByActorAndToken) ownsCacheFunc(f *func(coredb.Admire)) bool {
