...
type AdmireLoaderByActorAndPostSettings interface {
...
// AdmireLoaderByActorAndPostCacheSubscriptions
type AdmireLoaderByActorAndPostCacheSubscriptions struct {
	// AutoCacheWithKey is a function that returns the coredb.GetAdmireByActorIDAndPostIDParams cache key for a coredb.Admire.
	...
	AutoCacheWithKey func(coredb.Admire) coredb.GetAdmireByActorIDAndPostIDParams

	// AutoCacheWithKeys is a function that returns the []coredb.GetAdmireByActorIDAndPostIDParams cache keys for a coredb.Admire.
	...
	// that return a coredb.Admire. Loaders that return pointers or slices of coredb.Admire
	// will be dereferenced/iterated automatically, invoking this function with the base coredb.Admire type.
	AutoCacheWithKeys func(coredb.Admire) []coredb.GetAdmireByActorIDAndPostIDParams
...
func (l *AdmireLoaderByActorAndPost) setContext(ctx context.Context) {
	l.ctx = ctx
}

func (l *AdmireLoaderByActorAndPost) setWait(wait time.Duration) {
	l.wait = wait
}

func (l *AdmireLoaderByActorAndPost) setMaxBatch(maxBatch int) {
	l.maxBatch = maxBatch
}

func (l *AdmireLoaderByActorAndPost) setDisableCaching(disableCaching bool) {
	l.disableCaching = disableCaching
}

func (l *AdmireLoaderByActorAndPost) setPublishResults(publishResults bool) {
	l.publishResults = publishResults
}

func (l *AdmireLoaderByActorAndPost) setPreFetchHook(preFetchHook func(context.Context, string) context.Context) {
	l.preFetchHook = preFetchHook
}

func (l *AdmireLoaderByActorAndPost) setPostFetchHook(postFetchHook func(context.Context, string)) {
	l.postFetchHook = postFetchHook
}

// NewAdmireLoaderByActorAndPost creates a new AdmireLoaderByActorAndPost with the given settings, functions, and options
func NewAdmireLoaderByActorAndPost(
	settings AdmireLoaderByActorAndPostSettings, fetch func(ctx context.Context, keys []coredb.GetAdmireByActorIDAndPostIDParams) ([]coredb.Admire, []error),
	funcs AdmireLoaderByActorAndPostCacheSubscriptions,
	opts ...func(interface {
...
) *AdmireLoaderByActorAndPost {
	loader := &AdmireLoaderByActorAndPost{
	...
	loader.fetch = func(keys []coredb.GetAdmireByActorIDAndPostIDParams) ([]coredb.Admire, []error) {
		ctx := loader.ctx

		// Allow the preFetchHook to modify and return a new context
		if loader.preFetchHook != nil {
			ctx = loader.preFetchHook(ctx, "AdmireLoaderByActorAndPost")
		}

		results, errors := fetch(ctx, keys)

		if loader.postFetchHook != nil {
			loader.postFetchHook(ctx, "AdmireLoaderByActorAndPost")
		}
...
// AdmireLoaderByActorAndPost batches and caches requests
type AdmireLoaderByActorAndPost struct {
	// context passed to fetch functions
	ctx context.Context

	// this method provides the data for the loader
	fetch func(keys []coredb.GetAdmireByActorIDAndPostIDParams) ([]coredb.Admire, []error)
	...
	cache map[coredb.GetAdmireByActorIDAndPostIDParams]coredb.Admire

	// typed cache functions
	//subscribers []func(coredb.Admire)
	subscribers []admireLoaderByActorAndPostSubscriber
	...
	batch *admireLoaderByActorAndPostBatch
...
type admireLoaderByActorAndPostBatch struct {
	keys    []coredb.GetAdmireByActorIDAndPostIDParams
	data    []coredb.Admire
	error   []error
	closing bool
	done    chan struct{}
...
func (l *AdmireLoaderByActorAndPost) Load(key coredb.GetAdmireByActorIDAndPostIDParams) (coredb.Admire, error) {
...
func (l *AdmireLoaderByActorAndPost) LoadThunk(key coredb.GetAdmireByActorIDAndPostIDParams) func() (coredb.Admire, error) {
	l.mu.Lock()
	if !l.disableCaching {
	...
	if l.batch == nil {
		l.batch = &admireLoaderByActorAndPostBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *AdmireLoaderByActorAndPost) LoadAll(keys []coredb.GetAdmireByActorIDAndPostIDParams) ([]coredb.Admire, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *AdmireLoaderByActorAndPost) LoadAllThunk(keys []coredb.GetAdmireByActorIDAndPostIDParams) func() ([]coredb.Admire, []error) {
	results := make([]func() (coredb.Admire, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *AdmireLoaderByActorAndPost) Prime(key coredb.GetAdmireByActorIDAndPostIDParams, value coredb.Admire) bool {
	if l.disableCaching {
		return false
	}
	l.mu.Lock()
...
func (l *AdmireLoaderByActorAndPost) unsafePrime(key coredb.GetAdmireByActorIDAndPostIDParams, value coredb.Admire) bool {
	if l.disableCaching {
		return false
	}
	var found bool
...
func (l *AdmireLoaderByActorAndPost) Clear(key coredb.GetAdmireByActorIDAndPostIDParams) {
	if l.disableCaching {
		return
	}
	l.mu.Lock()
...
func (l *AdmireLoaderByActorAndPost) unsafeSet(key coredb.GetAdmireByActorIDAndPostIDParams, value coredb.Admire) {
	if l.cache == nil {
		l.cache = map[coredb.GetAdmireByActorIDAndPostIDParams]coredb.Admire{}
	}
	l.cache[key] = value
}
...
func (b *admireLoaderByActorAndPostBatch) keyIndex(l *AdmireLoaderByActorAndPost, key coredb.GetAdmireByActorIDAndPostIDParams) int {
...
func (b *admireLoaderByActorAndPostBatch) startTimer(l *AdmireLoaderByActorAndPost) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *admireLoaderByActorAndPostBatch) end(l *AdmireLoaderByActorAndPost) {
	b.data, b.error = l.fetch(b.keys)
	close(b.done)
}

type admireLoaderByActorAndPostSubscriber struct {
	cacheFunc func(coredb.Admire)
	mutex     *sync.Mutex
}

func (l *AdmireLoaderByActorAndPost) publishToSubscribers(value coredb.Admire) {
		...
		for i, subscription := range *l.subscriptionRegistry {
			if typedFunc, ok := subscription.(*func(coredb.Admire)); ok {
				...
				if !l.ownsCacheFunc(typedFunc) {
					l.subscribers = append(l.subscribers, admireLoaderByActorAndPostSubscriber{cacheFunc: *typedFunc, mutex: (*l.mutexRegistry)[i]})
				}
			}
		}
	})
...
func (l *AdmireLoaderByActorAndPost) registerCacheFunc(cacheFunc interface{}, mutex *sync.Mutex) {
...
func (l *AdmireLoaderByActorAndPost) ownsCacheFunc(f *func(coredb.Admire)) bool {
