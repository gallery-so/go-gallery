...
type NftsLoaderByAddressConfig struct {
	...
	Fetch func(keys []persist.Address) ([][]persist.NFT, []error)
...
type NftsLoaderByAddress struct {
	...
	cache map[persist.Address][]persist.NFT
...
func (l *NftsLoaderByAddress) Load(key persist.Address) ([]persist.NFT, error) {
...
func (l *NftsLoaderByAddress) LoadThunk(key persist.Address) func() ([]persist.NFT, error) {
	l.mu.Lock()
...
func (l *NftsLoaderByAddress) LoadAll(keys []persist.Address) ([][]persist.NFT, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *NftsLoaderByAddress) LoadAllThunk(keys []persist.Address) func() ([][]persist.NFT, []error) {
	results := make([]func() ([]persist.NFT, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *NftsLoaderByAddress) Prime(key persist.Address, value []persist.NFT) bool {
	l.mu.Lock()
	var found bool
...
func (l *NftsLoaderByAddress) unsafeSet(key persist.Address, value []persist.NFT) {
	if l.cache == nil {
		l.cache = map[persist.Address][]persist.NFT{}
	}
	l.cache[key] = value
}
