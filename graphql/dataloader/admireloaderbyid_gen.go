...
type AdmireLoaderByIDSettings interface {
...
// AdmireLoaderByIDCacheSubscriptions
type AdmireLoaderByIDCacheSubscriptions struct {
	// AutoCacheWithKey is a function that returns the persist.DBID cache key for a coredb.Admire.
	...
	AutoCacheWithKey func(coredb.Admire) persist.DBID

	// AutoCacheWithKeys is a function that returns the []persist.DBID cache keys for a coredb.Admire.
	...
	// that return a coredb.Admire. Loaders that return pointers or slices of coredb.Admire
	// will be dereferenced/iterated automatically, invoking this function with the base coredb.Admire type.
	AutoCacheWithKeys func(coredb.Admire) []persist.DBID
...
func (l *AdmireLoaderByID) setContext(ctx context.Context) {
	l.ctx = ctx
}

func (l *AdmireLoaderByID) setWait(wait time.Duration) {
	l.wait = wait
}

func (l *AdmireLoaderByID) setMaxBatch(maxBatch int) {
	l.maxBatch = maxBatch
}

func (l *AdmireLoaderByID) setDisableCaching(disableCaching bool) {
	l.disableCaching = disableCaching
}

func (l *AdmireLoaderByID) setPublishResults(publishResults bool) {
...
func NewAdmireLoaderByID(
	...
	funcs AdmireLoaderByIDCacheSubscriptions,
	opts ...func(interface {
...
// AdmireLoaderByID batches and caches requests
type AdmireLoaderByID struct {
	...
	subscribers []admireLoaderByIDSubscriber
...
type admireLoaderByIDBatch struct {
	keys    []persist.DBID
	data    []coredb.Admire
	error   []error
	closing bool
...
func (l *AdmireLoaderByID) Load(key persist.DBID) (coredb.Admire, error) {
...
func (l *AdmireLoaderByID) LoadThunk(key persist.DBID) func() (coredb.Admire, error) {
	l.mu.Lock()
	if !l.disableCaching {
	...
	if l.batch == nil {
		l.batch = &admireLoaderByIDBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *AdmireLoaderByID) LoadAll(keys []persist.DBID) ([]coredb.Admire, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *AdmireLoaderByID) LoadAllThunk(keys []persist.DBID) func() ([]coredb.Admire, []error) {
	results := make([]func() (coredb.Admire, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *AdmireLoaderByID) Prime(key persist.DBID, value coredb.Admire) bool {
	if l.disableCaching {
		return false
	}
	l.mu.Lock()
...
func (l *AdmireLoaderByID) unsafePrime(key persist.DBID, value coredb.Admire) bool {
	if l.disableCaching {
		return false
	}
	var found bool
...
func (l *AdmireLoaderByID) Clear(key persist.DBID) {
	if l.disableCaching {
		return
	}
	l.mu.Lock()
...
func (l *AdmireLoaderByID) unsafeSet(key persist.DBID, value coredb.Admire) {
	if l.cache == nil {
...
func (b *admireLoaderByIDBatch) keyIndex(l *AdmireLoaderByID, key persist.DBID) int {
...
func (b *admireLoaderByIDBatch) startTimer(l *AdmireLoaderByID) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *admireLoaderByIDBatch) end(l *AdmireLoaderByID) {
	b.data, b.error = l.fetch(b.keys)
	close(b.done)
}

type admireLoaderByIDSubscriber struct {
	cacheFunc func(coredb.Admire)
	mutex     *sync.Mutex
}

func (l *AdmireLoaderByID) publishToSubscribers(value coredb.Admire) {
		...
		for i, subscription := range *l.subscriptionRegistry {
			if typedFunc, ok := subscription.(*func(coredb.Admire)); ok {
				...
				if !l.ownsCacheFunc(typedFunc) {
					l.subscribers = append(l.subscribers, admireLoaderByIDSubscriber{cacheFunc: *typedFunc, mutex: (*l.mutexRegistry)[i]})
				}
			}
		}
	})
...
func (l *AdmireLoaderByID) registerCacheFunc(cacheFunc interface{}, mutex *sync.Mutex) {
...
func (l *AdmireLoaderByID) ownsCacheFunc(f *func(coredb.Admire)) bool {
