...
type UserFeedLoaderConfig struct {
	...
	Fetch func(keys []sqlc.GetUserFeedViewBatchParams) ([][]sqlc.FeedEvent, []error)
...
type UserFeedLoader struct {
	...
	cache map[sqlc.GetUserFeedViewBatchParams][]sqlc.FeedEvent
...
type userFeedLoaderBatch struct {
	keys    []sqlc.GetUserFeedViewBatchParams
	data    [][]sqlc.FeedEvent
	error   []error
	closing bool
	done    chan struct{}
...
func (l *UserFeedLoader) Load(key sqlc.GetUserFeedViewBatchParams) ([]sqlc.FeedEvent, error) {
...
func (l *UserFeedLoader) LoadThunk(key sqlc.GetUserFeedViewBatchParams) func() ([]sqlc.FeedEvent, error) {
	l.mu.Lock()
...
func (l *UserFeedLoader) LoadAll(keys []sqlc.GetUserFeedViewBatchParams) ([][]sqlc.FeedEvent, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *UserFeedLoader) LoadAllThunk(keys []sqlc.GetUserFeedViewBatchParams) func() ([][]sqlc.FeedEvent, []error) {
	results := make([]func() ([]sqlc.FeedEvent, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *UserFeedLoader) Prime(key sqlc.GetUserFeedViewBatchParams, value []sqlc.FeedEvent) bool {
	l.mu.Lock()
	var found bool
...
func (l *UserFeedLoader) Clear(key sqlc.GetUserFeedViewBatchParams) {
	l.mu.Lock()
	delete(l.cache, key)
	l.mu.Unlock()
}

func (l *UserFeedLoader) unsafeSet(key sqlc.GetUserFeedViewBatchParams, value []sqlc.FeedEvent) {
	if l.cache == nil {
		l.cache = map[sqlc.GetUserFeedViewBatchParams][]sqlc.FeedEvent{}
	}
	l.cache[key] = value
}
...
func (b *userFeedLoaderBatch) keyIndex(l *UserFeedLoader, key sqlc.GetUserFeedViewBatchParams) int {
