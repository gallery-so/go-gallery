...
type MediaLoaderByTokenIDCacheSubscriptions struct {
	...
	// will be dereferenced/iterated automatically, invoking this function with the base coredb.TokenMedia type.
	AutoCacheWithKey func(coredb.TokenMedia) persist.DBID

	// AutoCacheWithKeys is a function that returns the []persist.DBID cache keys for a coredb.TokenMedia.
	...
	// that return a coredb.TokenMedia. Loaders that return pointers or slices of coredb.TokenMedia
	// will be dereferenced/iterated automatically, invoking this function with the base coredb.TokenMedia type.
	AutoCacheWithKeys func(coredb.TokenMedia) []persist.DBID
...
func NewMediaLoaderByTokenID(
	...
	loader.fetch = func(keys []persist.DBID) ([]coredb.TokenMedia, []error) {
		ctx := loader.ctx
	...
	if !loader.disableCaching {
		...
		if funcs.AutoCacheWithKey != nil {
			cacheFunc := func(t coredb.TokenMedia) {
...
type MediaLoaderByTokenID struct {
	...
	fetch func(keys []persist.DBID) ([]coredb.TokenMedia, []error)
	...
	cache map[persist.DBID]coredb.TokenMedia
...
func (l *MediaLoaderByTokenID) LoadThunk(key persist.DBID) func() (coredb.TokenMedia, error) {
	...
	if !l.disableCaching {
		if it, ok := l.cache[key]; ok {
			l.mu.Unlock()
			return func() (coredb.TokenMedia, error) {
				return it, nil
			}
		}
	}
...
func (l *MediaLoaderByTokenID) LoadAll(keys []persist.DBID) ([]coredb.TokenMedia, []error) {
	...
	tokenMedias := make([]coredb.TokenMedia, len(keys))
	errors := make([]error, len(keys))
	for i, thunk := range results {
		tokenMedias[i], errors[i] = thunk()
	}
	return tokenMedias, errors
}
