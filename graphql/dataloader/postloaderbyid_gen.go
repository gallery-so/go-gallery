...
type PostLoaderByIDSettings interface {
...
// PostLoaderByIDCacheSubscriptions
type PostLoaderByIDCacheSubscriptions struct {
	// AutoCacheWithKey is a function that returns the persist.DBID cache key for a coredb.Post.
	// If AutoCacheWithKey is not nil, this loader will automatically cache published results from other loaders
	// that return a coredb.Post. Loaders that return pointers or slices of coredb.Post
	// will be dereferenced/iterated automatically, invoking this function with the base coredb.Post type.
	AutoCacheWithKey func(coredb.Post) persist.DBID

	// AutoCacheWithKeys is a function that returns the []persist.DBID cache keys for a coredb.Post.
	...
	// that return a coredb.Post. Loaders that return pointers or slices of coredb.Post
	// will be dereferenced/iterated automatically, invoking this function with the base coredb.Post type.
	AutoCacheWithKeys func(coredb.Post) []persist.DBID
...
func (l *PostLoaderByID) setContext(ctx context.Context) {
	l.ctx = ctx
}

func (l *PostLoaderByID) setWait(wait time.Duration) {
	l.wait = wait
}

func (l *PostLoaderByID) setMaxBatch(maxBatch int) {
	l.maxBatch = maxBatch
}

func (l *PostLoaderByID) setDisableCaching(disableCaching bool) {
	l.disableCaching = disableCaching
}

func (l *PostLoaderByID) setPublishResults(publishResults bool) {
	l.publishResults = publishResults
}

func (l *PostLoaderByID) setPreFetchHook(preFetchHook func(context.Context, string) context.Context) {
	l.preFetchHook = preFetchHook
}

func (l *PostLoaderByID) setPostFetchHook(postFetchHook func(context.Context, string)) {
	l.postFetchHook = postFetchHook
}

// NewPostLoaderByID creates a new PostLoaderByID with the given settings, functions, and options
func NewPostLoaderByID(
	settings PostLoaderByIDSettings, fetch func(ctx context.Context, keys []persist.DBID) ([]coredb.Post, []error),
	funcs PostLoaderByIDCacheSubscriptions,
	opts ...func(interface {
	...
	loader.fetch = func(keys []persist.DBID) ([]coredb.Post, []error) {
		...
		if loader.preFetchHook != nil {
			ctx = loader.preFetchHook(ctx, "PostLoaderByID")
		}

		results, errors := fetch(ctx, keys)

		if loader.postFetchHook != nil {
			loader.postFetchHook(ctx, "PostLoaderByID")
		}
...
// PostLoaderByID batches and caches requests
type PostLoaderByID struct {
	...
	subscribers []postLoaderByIDSubscriber
...
type postLoaderByIDBatch struct {
	keys    []persist.DBID
	data    []coredb.Post
	error   []error
	closing bool
...
func (l *PostLoaderByID) Load(key persist.DBID) (coredb.Post, error) {
...
func (l *PostLoaderByID) LoadThunk(key persist.DBID) func() (coredb.Post, error) {
	l.mu.Lock()
	if !l.disableCaching {
	...
	if l.batch == nil {
		l.batch = &postLoaderByIDBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *PostLoaderByID) LoadAll(keys []persist.DBID) ([]coredb.Post, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *PostLoaderByID) LoadAllThunk(keys []persist.DBID) func() ([]coredb.Post, []error) {
	results := make([]func() (coredb.Post, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *PostLoaderByID) Prime(key persist.DBID, value coredb.Post) bool {
	if l.disableCaching {
		return false
	}
	l.mu.Lock()
...
func (l *PostLoaderByID) unsafePrime(key persist.DBID, value coredb.Post) bool {
	if l.disableCaching {
		return false
	}
	var found bool
...
func (l *PostLoaderByID) Clear(key persist.DBID) {
	if l.disableCaching {
		return
	}
	l.mu.Lock()
...
func (l *PostLoaderByID) unsafeSet(key persist.DBID, value coredb.Post) {
	if l.cache == nil {
...
func (b *postLoaderByIDBatch) keyIndex(l *PostLoaderByID, key persist.DBID) int {
...
func (b *postLoaderByIDBatch) startTimer(l *PostLoaderByID) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *postLoaderByIDBatch) end(l *PostLoaderByID) {
	b.data, b.error = l.fetch(b.keys)
	close(b.done)
}

type postLoaderByIDSubscriber struct {
	cacheFunc func(coredb.Post)
	mutex     *sync.Mutex
}

func (l *PostLoaderByID) publishToSubscribers(value coredb.Post) {
		...
		for i, subscription := range *l.subscriptionRegistry {
			if typedFunc, ok := subscription.(*func(coredb.Post)); ok {
				...
				if !l.ownsCacheFunc(typedFunc) {
					l.subscribers = append(l.subscribers, postLoaderByIDSubscriber{cacheFunc: *typedFunc, mutex: (*l.mutexRegistry)[i]})
				}
			}
		}
	})
...
func (l *PostLoaderByID) registerCacheFunc(cacheFunc interface{}, mutex *sync.Mutex) {
...
func (l *PostLoaderByID) ownsCacheFunc(f *func(coredb.Post)) bool {
