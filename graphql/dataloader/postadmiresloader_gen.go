...
type PostAdmiresLoaderSettings interface {
...
func (l *PostAdmiresLoader) setContext(ctx context.Context) {
	l.ctx = ctx
}

func (l *PostAdmiresLoader) setWait(wait time.Duration) {
	l.wait = wait
}

func (l *PostAdmiresLoader) setMaxBatch(maxBatch int) {
	l.maxBatch = maxBatch
}

func (l *PostAdmiresLoader) setDisableCaching(disableCaching bool) {
	l.disableCaching = disableCaching
}

func (l *PostAdmiresLoader) setPublishResults(publishResults bool) {
	l.publishResults = publishResults
}

func (l *PostAdmiresLoader) setPreFetchHook(preFetchHook func(context.Context, string) context.Context) {
	l.preFetchHook = preFetchHook
}

func (l *PostAdmiresLoader) setPostFetchHook(postFetchHook func(context.Context, string)) {
	l.postFetchHook = postFetchHook
}

// NewPostAdmiresLoader creates a new PostAdmiresLoader with the given settings, functions, and options
func NewPostAdmiresLoader(
	settings PostAdmiresLoaderSettings, fetch func(ctx context.Context, keys []coredb.PaginateAdmiresByPostIDBatchParams) ([][]coredb.Admire, []error),
	opts ...func(interface {
	...
	loader.fetch = func(keys []coredb.PaginateAdmiresByPostIDBatchParams) ([][]coredb.Admire, []error) {
		ctx := loader.ctx

		// Allow the preFetchHook to modify and return a new context
		if loader.preFetchHook != nil {
			ctx = loader.preFetchHook(ctx, "PostAdmiresLoader")
		}

		results, errors := fetch(ctx, keys)

		if loader.postFetchHook != nil {
			loader.postFetchHook(ctx, "PostAdmiresLoader")
		}
...
// PostAdmiresLoader batches and caches requests
type PostAdmiresLoader struct {
	// context passed to fetch functions
	ctx context.Context

	// this method provides the data for the loader
	fetch func(keys []coredb.PaginateAdmiresByPostIDBatchParams) ([][]coredb.Admire, []error)
	...
	cache map[coredb.PaginateAdmiresByPostIDBatchParams][]coredb.Admire

	// typed cache functions
	//subscribers []func([]coredb.Admire)
	subscribers []postAdmiresLoaderSubscriber
...
type postAdmiresLoaderBatch struct {
	keys    []coredb.PaginateAdmiresByPostIDBatchParams
	data    [][]coredb.Admire
	error   []error
	closing bool
	done    chan struct{}
...
func (l *PostAdmiresLoader) Load(key coredb.PaginateAdmiresByPostIDBatchParams) ([]coredb.Admire, error) {
...
func (l *PostAdmiresLoader) LoadThunk(key coredb.PaginateAdmiresByPostIDBatchParams) func() ([]coredb.Admire, error) {
	l.mu.Lock()
	if !l.disableCaching {
	...
	if l.batch == nil {
		l.batch = &postAdmiresLoaderBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *PostAdmiresLoader) LoadAll(keys []coredb.PaginateAdmiresByPostIDBatchParams) ([][]coredb.Admire, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *PostAdmiresLoader) LoadAllThunk(keys []coredb.PaginateAdmiresByPostIDBatchParams) func() ([][]coredb.Admire, []error) {
	results := make([]func() ([]coredb.Admire, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *PostAdmiresLoader) Prime(key coredb.PaginateAdmiresByPostIDBatchParams, value []coredb.Admire) bool {
	if l.disableCaching {
		return false
	}
	l.mu.Lock()
...
func (l *PostAdmiresLoader) Clear(key coredb.PaginateAdmiresByPostIDBatchParams) {
	if l.disableCaching {
		return
	}
	l.mu.Lock()
...
func (l *PostAdmiresLoader) unsafeSet(key coredb.PaginateAdmiresByPostIDBatchParams, value []coredb.Admire) {
	if l.cache == nil {
		l.cache = map[coredb.PaginateAdmiresByPostIDBatchParams][]coredb.Admire{}
	}
	l.cache[key] = value
}
...
func (b *postAdmiresLoaderBatch) keyIndex(l *PostAdmiresLoader, key coredb.PaginateAdmiresByPostIDBatchParams) int {
...
func (b *postAdmiresLoaderBatch) startTimer(l *PostAdmiresLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *postAdmiresLoaderBatch) end(l *PostAdmiresLoader) {
	b.data, b.error = l.fetch(b.keys)
	close(b.done)
}

type postAdmiresLoaderSubscriber struct {
	cacheFunc func(coredb.Admire)
	mutex     *sync.Mutex
}

func (l *PostAdmiresLoader) publishToSubscribers(value []coredb.Admire) {
		...
		for i, subscription := range *l.subscriptionRegistry {
			if typedFunc, ok := subscription.(*func(coredb.Admire)); ok {
				...
				if !l.ownsCacheFunc(typedFunc) {
					l.subscribers = append(l.subscribers, postAdmiresLoaderSubscriber{cacheFunc: *typedFunc, mutex: (*l.mutexRegistry)[i]})
				}
			}
		}
	})
...
func (l *PostAdmiresLoader) registerCacheFunc(cacheFunc interface{}, mutex *sync.Mutex) {
...
func (l *PostAdmiresLoader) ownsCacheFunc(f *func(coredb.Admire)) bool {
