...
func ResponseReporter(log bool, trace bool) func(ctx context.Context, next gqlgen.ResponseHandler) *gqlgen.Response {
	return func(ctx context.Context, next gqlgen.ResponseHandler) *gqlgen.Response {
		...
		// Unique ID to make finding this log entry easy
		...
		if span != nil {
			...
			if len(response.Data) <= maxBytes {
				...
				placeholder := fmt.Sprintf("response omitted because it is too large (%d bytes)", len(response.Data))
				if log {
					placeholder += fmt.Sprintf(", but it should be accessible by searching logs for this unique ID: %s", responseLocatorId)
				}
				eventData["scrubbedQuery"] = placeholder
			}
	...
	return func(ctx context.Context, next gqlgen.OperationHandler) gqlgen.ResponseHandler {
		...
		if span != nil {
			...
			if len(scrubbedQuery) <= maxBytes {
				...
				placeholder := fmt.Sprintf("scrubbedQuery omitted because it is too large (%d bytes)", len(scrubbedQuery))
				if log {
					placeholder += fmt.Sprintf(", but it should be accessible by searching logs for this unique ID: %s", requestLocatorId)
				}
				eventData["scrubbedQuery"] = placeholder
			}
