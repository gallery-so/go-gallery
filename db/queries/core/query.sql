...
update users set primary_wallet_id = @wallet_id where users.id = @user_id and (select exists(select 1 from wallets where wallets.id = @wallet_id and wallets.owner_user_id = @user_id and wallets.deleted = false));
