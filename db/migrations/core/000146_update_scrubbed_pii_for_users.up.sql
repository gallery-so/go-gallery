...
create view scrubbed_pii.for_users as (
    ...
    -- <username>-unverified@dummy-email.gallery.so for users who have email addresses, null otherwise
    scrubbed_unverified_email_address as (
        select u.id as user_id,
               case
                   when p.pii_unverified_email_address is not null then u.username_idempotent || '-unverified@dummy-email.gallery.so'
               end as scrubbed_address from users u, pii.for_users p
        where u.id = p.user_id
    ),

    -- <username>@dummy-email.gallery.so for users who have verified email addresses, null otherwise
    scrubbed_verified_email_address as (
        select u.id as user_id,
               case
                   when p.pii_verified_email_address is not null then u.username_idempotent || '@dummy-email.gallery.so'
               end as scrubbed_address from users u, pii.for_users p
        where u.id = p.user_id
    )
    ...
    union all
    select p.user_id, unverified_email.scrubbed_address, p.deleted, s.scrubbed_socials, verified_email.scrubbed_address
