-- Drop the community_galleries view because it depends on the displayable column
drop materialized view community_galleries;

-- Get rid of displayable and re-add it as a "not null default true" boolean. This is trivial and fast,
-- compared to adding it as a generated column, which would lock the table for minutes.
alter table tokens drop column displayable;
alter table tokens add displayable boolean not null default true;

-- Re-add the only index that uses displayable
create index tokens_owner_user_id_displayable_idx on tokens (owner_user_id, displayable) where (deleted = false);
