...
alter table tokens add column if not exists is_owned boolean not null
...
create index if not exists tokens_owner_user_id_is_owned_idx on tokens (owner_user_id, is_owned) where deleted = false;
