...
alter table tokens add column if not exists is_displayable boolean not null
...
create index if not exists tokens_owner_user_id_is_displayable_idx on tokens (owner_user_id, is_displayable) where deleted = false;
