...
with first_posts as (
    select id
    from posts p
    where not exists(select 1 from posts where created_at < p.created_at and p.actor_id = actor_id limit 1)
)
update posts set is_first_post = true
from first_posts
where first_posts.id = posts.id;
